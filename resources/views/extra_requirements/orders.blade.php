                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            ...
                            <div>
                                @php 
                                    $verifiedPayment = $order->invoice->payment_status == 'paid' ? $order->invoice->payments->first() : null;
                                ...
                                <h5><strong>Amount Received: </strong>{{  $order->invoice->currency}} {{  $order->invoice->amount_paid ?? 'N/A' }} </h5>
                                
                                <p><strong>Payment Date: </strong> {{  $order->invoice->updated_at ? \Carbon\Carbon::parse($order->invoice->updated_at)->format('d M Y') : 'N/A' }}</p>
                                @endif
                                @if($verifiedPayment)
                        ...
                        @if($order->invoice->payment_status == 'unpaid')
                            <form action="/payment/{{ $order->invoice->invoice_no}}" method="GET" style="display:inline;">
                                {{-- <input type="hidden" name="invoice_no" value="{{ $order->invoice->invoice_no ?? 'N/A' }}"> --}}
